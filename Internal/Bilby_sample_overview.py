...
# script info
__script__.title = '<Bilby Sample Measurements Overview>'
...
def __run_script__(fns):
    ...
    if (fns is None or len(fns) == 0) :
        ...
        print 'Filename, SampleName, SaveTime, MeasurementTime, TotalCounts, UnixTime'
        ...
        for fn in fns:
            ...
            ds.__iDictionary__.addEntry('SaveTime', 'entry1/end_time')
            ds.__iDictionary__.addEntry('MeasurementTime', 'entry1/instrument/detector/time')
            ds.__iDictionary__.addEntry('TotalCounts', 'entry1/data/total_counts')
            ds.__iDictionary__.addEntry('UnixTime', 'entry1/instrument/detector/start_time')
            
            #print type(ds.SampleName), len(ds.SampleName)            
            #print type(ds.SaveTime), len(ds.SaveTime)
            #print type(ds.MeasurementTime), len(ds.MeasurementTime)
            
            #if type(ds.SampleName) == 'instance':
            #    print 'hello'
                
            #print type(ds.TotalCounts), len(ds.TotalCounts)
            #print type(ds.UnixTime), len(ds.UnixTime)
            def is_number(s):
                try:
                    float(s)
                    return True
            ...
            if is_number(ds.MeasurementTime):
                print filename,',', str(ds.SampleName),',', str(ds.SaveTime), ',',str(int(ds.MeasurementTime)), ',',str(ds.TotalCounts),',',str(ds.UnixTime)
            else:
                print filename,', IGNORE TEST'
