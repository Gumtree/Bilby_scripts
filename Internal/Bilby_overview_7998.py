...
__script__.version = '2019-12-20 include 1T magnet + cryostat'

__FOLDER_PATH__ = 'U:/data/proposal/07879/'
...
SamplePosition_tick = Par('bool', False)
...
TotalCounts_tick = Par('bool', False)
TotalCounts_tick.title = '   Total Counts'
Att_tick = Par('bool', False)
...
L1_tick = Par('bool', True)
L1_tick.title = '  L1'
L2det_tick = Par('bool', True)
L2det_tick.title = '  L2 rear'
L2curtainLR_tick = Par('bool', True)
L2curtainLR_tick.title = '  L2 curtain LR'
L2curtainUD_tick = Par('bool', True)
L2curtainUD_tick.title = '  L2 curtain UD'
curtainL_tick = Par('bool', True)
curtainL_tick.title = '  curtainL'
curtainR_tick = Par('bool', True)
curtainR_tick.title = '  curtainR'
curtainU_tick = Par('bool', True)
curtainU_tick.title = '  curtainU'
curtainD_tick = Par('bool', True)
...
Sample_temp_tick = Par('bool', True) # old data format
Sample_temp_tick.title = '  SampleTemp, K'
Electrical_field_tick = Par('bool', True) # old data format
Electrical_field_tick.title = '  Nominal_outp_current '

g0 = Group('Electrical magnet settings')
g0.numColumns = 3
g0.add(Sample_temp_tick, Electrical_field_tick)

# Seventh block Reactor
Reactor_power_tick = Par('bool', True) # old data format
Reactor_power_tick.title = '  ReactorPower, K'
ColdSourceTemp_tick = Par('bool', True) # old data format
...
exportfilename = Par('string', 'test_7879_7998')
...
def __run_script__(fns):
    ...
    if (fns is None or len(fns) == 0) :
        ...
        for fn in fns:
            ...
            if Electrical_field_tick.value:          #~~~~~~~~~~~~~~~~~~~~~
                header.append('Nominal_output_current')                
                try:   
                    a = str(ds['entry1/sample/ma1/sensor/nominal_outp_current'])
